...
namespace NGramSearchTest
{
    public class NoisyNgramTest
    {
        ...
        public void NoisyWordTest()
        {
            ...
            var abcResult = index.SearchNramCountWithFrequency("abc");
            Assert.Equal('a', abcResult.First().Id);
            Assert.Equal(3, abcResult.First().Similarity); // "_ab", "abc", "bc_"

            var agResult = index.SearchNramCountWithFrequency("ag");
        ...
        public void NoisyWordInOneItemTest()
        {
            ...
            var agResult = index.SearchNramCountWithFrequency("agx");//  "_ag"  -> 1/2

            Assert.Equal(0.5, agResult.First().Similarity, 8);

            var ag2Result = index.SearchNramCountWithFrequency("agx agy"); // "_ag" -> 2/2
        ...
        public void TwoNoisyWordInItemTest()
        {
            ...
            var ag2Result = index.SearchNramCountWithFrequency("agx agy");
